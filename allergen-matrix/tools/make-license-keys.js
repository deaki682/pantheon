/*
 * make-license-keys.js — offline license-key mint for Menu Allergen Matrix Builder Pro
 * -----------------------------------------------------------------------------------
 ...
 *
 * Outputs (all written next to this script, under tools/):
 *   signing-key.json   the keypair — PRIVATE, gitignored. Back it up; guard it.
 *   license-keys.csv   the batch of keys — upload this list to Payhip / Gumroad /
 *                      Lemon Squeezy as your "license key" pool. Gitignored.
 *   public-key.json    the public JWK (for reference).
 * It also rewrites the PUBLIC_JWK line inside ../license.js in place.
 *
 * Key format:  ALGN-<base64url( serial[5] || signature[64] )>
 * Signed message: "allergen-matrix-pro:v1:" + serialHex
 *
 * IMPORTANT: if you generate a NEW keypair (delete signing-key.json), every key you
 * previously sold stops verifying. Keep signing-key.json safe and reuse it to mint
 * more keys as you sell out.
 */
'use strict';
...
if (fs.existsSync(KEY_FILE)) {
  ...
  var privateJwk = priv.export({ format: 'jwk' });
  fs.writeFileSync(KEY_FILE, JSON.stringify({ privateJwk: privateJwk, publicJwk: publicJwk }, null, 2));
...
function mint() {
  ...
  var msg = Buffer.from(MSG_PREFIX + serialHex, 'utf8');
  var sig = crypto.sign('sha256', msg, { key: priv, dsaEncoding: 'ieee-p1363' }); // raw 64-byte r||s
  var payload = Buffer.concat([serial, sig]); // 69 bytes
  return PREFIX + '-' + b64url(payload);
}
...
// ---- bake the public key into license.js ----
...
if (patched === src) {
  console.warn('WARNING: could not find KEY_START/KEY_END markers in license.js — public key NOT updated.');
} else {
  fs.writeFileSync(LICENSE_JS, patched);
  console.log('Embedded public key into ' + path.relative(path.join(HERE, '..'), LICENSE_JS) + '.');
}
