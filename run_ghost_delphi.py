...
Runs Delphi's momentum screen (delphi.screener.score_universe) against the
full 118-name universe but places NO orders and touches NO real sleeve.
Opens a paper position for EVERY priceable universe candidate — not just the
top 10 the mechanical system would actually hold — marks the book to market,
grades matured positions (~90-day momentum holding window), and emits a
signal report (momentum terciles).
...
from delphi.ghost import (
    candidates_to_ghost, signal_report,
...
# ── Build candidates: ALL ranked universe names (not just the top 10) ──
# score_universe already filters to "above 20-day MA" (the mechanical entry
# gate); everything below the MA is implicitly excluded from the momentum
# ranking and wouldn't be a candidate the live system would ever hold, so we
# treat "ranked" as the full candidate pool for signal testing.
candidates = candidates_to_ghost(ranked, price_lookup, horizon_days=DEFAULT_DELPHI_HORIZON_DAYS)
candidates = [c for c in candidates if c["price"] >= MIN_PRICE]
log.info("Priced candidates (>= $%.0f): %d / %d ranked", MIN_PRICE, len(candidates), len(ranked))
