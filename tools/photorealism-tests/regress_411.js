...
async function run(br, v, scale, report) {
  ...
  try {
    ...
    const after = await pg.evaluate(() => { const v = document.getElementById('viewport').getBoundingClientRect();
      // the reference is drawn on one of TWO canvases now - refImg when the
      // world-sized composite is showing, refHi when the photograph itself
      // is. Reading refImg blind measures a display:none element and every
      // rect comes back zero.
      const w = (typeof refCanvas==='function' ? refCanvas()
                 : document.getElementById('refImg')).getBoundingClientRect();
