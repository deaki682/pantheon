// magnified side-by-side of the same region, old | new, with the diff
  ...
  for(let y=0;y<h;y++){const ft=raw[ro++],line=raw.slice(ro,ro+stride);ro+=stride;
    for(let x=0;x<stride;x++){const a=x>=ch?out[o+x-ch]:0,bb=y>0?out[o-stride+x]:0,
      ...
      if(ft===1)v+=a;else if(ft===2)v+=bb;else if(ft===3)v+=(a+bb)>>1;
        ...
        v+=(pa<=pb&&pa<=pc)?a:(pb<=pc?bb:c);} out[o+x]=v&255;}
    o+=stride;}
  return {w,h,ch,d:out};
}
function writePNG(f,w,h,rgba){
  ...
  let tb=null; const crc=(buf)=>{if(!tb){tb=[];for(let n=0;n<256;n++){let c=n;
    for(let k=0;k<8;k++)c=c&1?0xedb88320^(c>>>1):c>>>1; tb[n]=c>>>0;}}
    let c=0xffffffff; for(const b of buf)c=tb[(c^b)&255]^(c>>>8); return (c^0xffffffff)>>>0;};
  const ihdr=Buffer.alloc(13); ihdr.writeUInt32BE(w,0); ihdr.writeUInt32BE(h,4);
  ihdr[8]=8; ihdr[9]=6;
  fs.writeFileSync(f, Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),
    chunk('IHDR',ihdr), chunk('IDAT',idat), chunk('IEND',Buffer.alloc(0))]));
}
const [fa,fb,out,X,Y,W,H,Z]=process.argv.slice(2);
const A=readPNG(fa), B=readPNG(fb);
const x0=+X,y0=+Y,cw=+W,chh=+H,z=+Z;
const gap=14, lw=cw*z;
const outW=lw*3+gap*2, outH=chh*z;
const buf=Buffer.alloc(outW*outH*4,255);
const put=(px,py,r,g,b)=>{const o=(py*outW+px)*4; buf[o]=r;buf[o+1]=g;buf[o+2]=b;buf[o+3]=255;};
for(let y=0;y<chh;y++)for(let x=0;x<cw;x++){
  const ia=((y0+y)*A.w+(x0+x))*A.ch, ib=((y0+y)*B.w+(x0+x))*B.ch;
  const d=Math.min(255, Math.max(Math.abs(A.d[ia]-B.d[ib]),
    Math.abs(A.d[ia+1]-B.d[ib+1]), Math.abs(A.d[ia+2]-B.d[ib+2]))*6);
  for(let dy=0;dy<z;dy++)for(let dx=0;dx<z;dx++){
    put(x*z+dx, y*z+dy, A.d[ia],A.d[ia+1],A.d[ia+2]);
    put(lw+gap+x*z+dx, y*z+dy, B.d[ib],B.d[ib+1],B.d[ib+2]);
    put(lw*2+gap*2+x*z+dx, y*z+dy, 255-d, 255-d, 255);
  }
}
writePNG(out,outW,outH,buf);
console.log(out+'  '+outW+'x'+outH+'   left=v425  middle=now  right=diff(x6)');
