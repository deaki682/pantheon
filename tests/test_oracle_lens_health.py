...
def test_non_universe_lens_empty_always_flagged():
    # smart_money / activist don't depend on the equity universe — zero is
    # suspect even on a tiny capped run.
    assert assess_lens("smart_money", 0, universe_size=5).ok is False
    assert assess_lens("activist_13d", 0, universe_size=5).ok is False
...
def test_assess_and_all_ok_reflects_the_failing_lens():
    # This is exactly the broken run we salvaged: 3 lenses dead, quality fine.
    ...
    assert failed == {"insiders", "smart_money", "activist_13d"}
