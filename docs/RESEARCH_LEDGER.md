...
|---|---|---|---|---|
| 2026-07-04 | House lab: insider-cluster replay, survivorship correction attempt | **NO MATERIAL CHANGE** (immaterial, not "measured and confirmed"): only 1 of 42 previously-unpriceable events resolved genuinely in Sharadar SEP (PHXE-P, +19.4% excess at 6mo); 35/42 unmatched (OTC/pink-sheet names outside Sharadar's exchange-listed universe) and 4/7 raw resolver hits were FALSE ticker-recycling matches caught by manual window verification before use (NORD/UTGN/CHUC/PALX — a real gap in `resolve_ticker`'s single-candidate fallback). Corrected: 6mo −4.12%→−4.08% (n 612→613), 12mo unchanged at −6.38% (n=291, no new event reached maturity). Original 2026-07-03 REFUTED verdict stands untouched. | Backlog #6 closed as vendor-blocked, not fully answered — the 41 unresolved names' true outcomes remain unknown; a genuine correction needs OTC-coverage data. Filed follow-up: fix `resolve_ticker`'s window-fallback gap before any study trusts an unverified single-candidate hit. | [prereg](lab_prereg_cluster_survivorship_correction.md) · [results](lab_results_cluster_survivorship_correction.md) |
