...
|---|----------|------------------|-------------|----------|----------|
| 1 | **Quiet-cluster ghost**: do insider clusters in names with NO news and NO price move (the "nobody noticed" subset) outperform the full cluster population that measured -6.4%/yr? | Whether a filtered cluster strategy earns a ghost, after the raw signal was refuted | Fresh Form 4 population (fresh-data prereg — the replay dataset is spent), news/price screens at filing date | **Prereg committed 2026-07-04**: [docs/lab_prereg_quiet_cluster_ghost.md](lab_prereg_quiet_cluster_ghost.md); `quiet_cluster_ghost` registered + preregistered in the lab (`hypotheses_ever` 91→92). Fresh window is 2026-06-01 onward (zero overlap with the spent replay). Flags a real gap: `shared.lab`'s registry has no path from `preregistered` straight to forward-tracking when no backtest is possible (this hypothesis has none by design) — running as a measurement study outside the registry (paper entries via `shared.ghost` under this slug), per the prereg's option (b). **First population scan run 2026-07-04** (`run_quiet_cluster_scan.py`, reusable/resumable): all 2,408 daily-index candidates 2026-06-01..2026-07-04 checked; 90 qualifying clusters found (5 quiet, 85 loud comparison-arm), 68 ghost-opened (22 unpriceable via Sharadar — mostly warrants/OTC/preferred, disclosed not discarded). n=5 quiet events is far below the n≥30 floor — this will take well over a year of real time to accrue per the prereg's own math; future `/lab` sessions re-run the same script (full-window rescan, resumable per-CIK progress) to keep the population growing and grade matured entries as horizons elapse. | High |
| 2 | **Quality-lens validation**: does Oracle's quality score predict anything out-of-sample? | Whether quality stays in the cohort-2 selection stack or gets dropped | Fresh XBRL fundamentals + forward returns; prereg BEFORE cohort-2 selection (~11 months away, but prereg soon locks honesty) | **Prereg committed 2026-07-04**: [docs/lab_prereg_quality_lens_validation.md](lab_prereg_quality_lens_validation.md) — measurement study, skips the registry (no `hypotheses_ever` increment) per the liturgy's own example. Population frozen to the 100-name 2026-06-28 quarterly-screen output (Oracle's actual insider-gated, market-cap-filtered pond), `lenses.quality` taken verbatim (18/100 exact zeros disclosed, not excluded). **Ghost entries opened 2026-07-04** (`run_quality_lens_validation.py`, one-shot — not rolling): all 100 priced and opened (entry 2026-07-02, the last trading session before the July 4th holiday weekend), tercile cut points ~0.32/~0.52. Graded once at the 12-month horizon (~2027-07), timed to land just before cohort-2 selection; a future `/lab` session runs `shared.ghost.numeric_tercile_stats` on the frozen `quality` feature and writes the results doc + ledger row. | High |
