...
| 4 | **Delphi point-in-time universe**: rebuild the 118-name momentum backtest with index membership as-of each date; does the +85% survive? | Whether Delphi's backtest evidence is real or survivorship artifact — bears directly on her capital ceiling | Historical index membership lists (point-in-time), delisting-inclusive bars | **RE-BLOCKED 2026-07-04** — the "reconstruct from SEP market-cap fields" plan assumed `SHARADAR/DAILY` was queryable; verified live it returns 200/zero-rows (not entitled), and `SF1` gives only a trailing-2-year annual slice. Same wall as backlog #9 — see [gauntlet status](lab_gauntlet_engine_status_2026-07-04.md). Needs the same operator decision (buy the entitlement, accept a dollar-volume proxy, or another PIT membership source) before this can rebuild honestly | High — blocked pending #9's data-source decision |
...
| 9 | **The Gauntlet — strategy-factory simulation**: build a decade+ daily-bar simulation on the full Sharadar panel (~22k names incl. delisted, point-in-time market-cap universes, cost model) and run a PRE-COMMITTED grid of hundreds of simple strategy variants through it | A quantitative graveyard (which idea families are dead across 12 years x 22k names) + at most a few holdout survivors that earn forward tests — the empirical floor under every god's priors | Sharadar SEP bulk export; EDGAR event caches for event-driven families | **(a) DONE 2026-07-04** — engine (`shared/gauntlet.py`) + bulk fetch (`shared.sharadar.fetch_sep_bulk_range`) built and tested; see [status doc](lab_gauntlet_engine_status_2026-07-04.md). **NEW BLOCKER found while building (a):** no point-in-time market-cap source is entitled on the current Nasdaq Data Link subscription — `SHARADAR/DAILY` returns HTTP 200 with zero rows for every query tried, `SF1` gives only a trailing-2-year annual (MRY) slice, no MRQ/ARQ, no bulk cross-section. Phase (b)'s factory prereg cannot honestly define a market-cap universe until the operator picks: buy the DAILY/full-SF1 entitlement, accept the dollar-volume liquidity proxy (`dollar_volume_pit_universe`, built), or source membership elsewhere. Design prereg still REQUIRED before first run: full grid enumerated (hypotheses_ever += grid size), in-sample/holdout split frozen (holdout touched ONCE by survivors only), deflated-Sharpe / multiple-testing-corrected bar (implemented: `deflated_sharpe_ratio`), realistic costs at house size (implemented: `CostModel`). Survivors still face the standard >=20-grade forward gate | **GREEN-LIT by operator 2026-07-04** — top of queue, phase (a) done. Build order across lab sessions: (a) engine + SEP bulk pipeline **DONE**, (b) factory prereg — **BLOCKED on operator market-cap-source decision above**, (c) in-sample screen, (d) holdout pass for survivors only, (e) forward tests. #4 (Delphi PIT) hits the identical blocker and should ride the same engine/data pipeline rather than duplicate it |
