...
def _to_utc_naive(dt: datetime) -> datetime:
    """Normalize an aware or naive datetime to naive UTC.

    Cadence files mix naive UTC timestamps (datetime.utcnow().isoformat(),
    e.g. mark_run's default) with timezone-aware UTC timestamps
    (datetime.now(timezone.utc).isoformat(), written by some callers) —
    subtracting one from the other raises TypeError. Normalize both sides
...
def days_since(path: str, key: str, *, now: datetime | None = None) -> float | None:
    ...
    if not raw:
        return None
    try:
        ts = _to_utc_naive(datetime.fromisoformat(raw))
    except ValueError:
        return None
