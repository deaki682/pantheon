...
| `/midas-scan` | **CLOSED PERMANENTLY 2026-09-01** (directive D3, operator-signed) — NEVER dispatched | The convergence A/B program is closed: thesis 0-for-3, finalist pool −2.4%/wk at a 30% hit rate, and 7 consecutive weeks unable to produce a valid head-to-head, so the preregistered comparison can never complete. `cache/midas_scan.json` + the scan caches are frozen as history. The idea may return ONLY through the lab front door with a new preregistered mechanism |
...
| `/midas-ghost` | **CLOSED PERMANENTLY 2026-09-01** (directive D3, operator-signed) — NEVER dispatched | The live-vs-legacy A/B race is closed at 20 graded rows with non-monotonic terciles. `cache/ghost_midas_*` is frozen as history; nothing is deleted, nothing is graded further |
| `/proteus` | `should_run("cache/proteus_cadence.json", "session", 1)` — one session per day, any day. Normal `is_paused` semantics apply — a pause file Proteus didn't write means the operator is holding him | **Proteus v3 (operator override 2026-08-12 — docs/proteus_v3_charter.md, plus Amendment I signed 2026-09-01):** one mandate, make money on his own carried-over sleeve. v1 and v2 are both scrapped; apply neither's rules (no `pending_funding`, no seasonal PEAD mode, no sizing ladders, no probe caps, no grading checkpoint). Zeus dispatches him and does not read his logic |
| `/proteus-lab` | **RETIRED with Proteus v1 (2026-07-11)** — never dispatched | v2 and v3 are both exempt from the lab ratchet; Proteus runs his own education inside `/proteus`. The house `/lab` is unaffected |
| `/argus` | `should_run("cache/argus_cadence.json", "session", 0.8)` (0.8 not 1: the session-END stamp drifts past the next day's fixed wake minute — a 1.0 gate skips whole days, observed 2026-09-11) — one session per day, ANY day (crypto is 24/7; weekends are his most valuable watches). **LIVE 2026-09-08** (launch override, Amendment 0 in docs/argus_charter_draft_2026-09-08.md): $1k sleeve, compounding. Kill-switch check is part of every session | The hundred-eyed watchman: spot-crypto forced-flow striker. Most sessions are watch-only (zero orders, full journal). Owns only `cache/argus_*` |
...
2. **Check cadence files.** Read the cadence/state files to decide what's due:
   ...
   # NOTE: there is no midas_scan_due. The Midas research program (/midas-scan,
   # /midas-ghost, the A/B and its death clock) is CLOSED PERMANENTLY as of
   # 2026-09-01 (directive D3, operator-signed). Do not reintroduce the cadence
   # lookup — an absent variable is the guard.
...
3. **Build the dispatch list.** Apply the conditions from the table above:
   ...
   **Conditional:**
   ...
   - `/midas-scan`, `/midas-ghost` — CLOSED PERMANENTLY 2026-09-01 (directive D3, operator-signed). NEVER dispatched, on any day, under any cadence. Their caches are frozen history
   ...
   - `/proteus` — if `(not proteus_paused)` AND `should_run("cache/proteus_cadence.json", "session", 1)`. **Proteus v3, launched 2026-08-12 (docs/proteus_v3_charter.md)** — one session per day, fully autonomous on his own sleeve (and only his own).
   - `/proteus-lab` — RETIRED with Proteus v1 (2026-07-11). Never dispatched; Proteus educates himself inside `/proteus`.
   ...
   **Ghosts:** `/oracle-ghost` runs after `/oracle` (its parent still
   ...
   own daily cadences, listed in group 3 below. `/midas-ghost` is CLOSED, not
   parentless: it is never dispatched again.
...
4. **Dispatch.** Run the due skills. Independent gods can run in parallel
   ...
   **Parallel group 1** (independent gods, run together — only after group 0):
   ...
   - `/proteus` (if due — v3, fully autonomous; owns only `cache/proteus_*`, so he parallelizes safely within this group)
...
system to fire once per hour during waking hours (or 24/7 if you want the
weekend `/lab` and `/proteus` runs to fire automatically).
...
- Weekend dispatches: only `/proteus` (daily cadence includes weekends as build/research days; markets closed means no orders) and `/lab` (house research lab, paper only) run. No `/trinity`, `/plutus`, `/hermes`, or `/midas` on weekends (markets closed — nothing to trade). `/achilles`, `/nemesis`, and `/proteus-lab` are folded/retired and never dispatched; `/midas-scan` and `/midas-ghost` are CLOSED and never dispatched.
- Midas is fully closed. Live trading retired 2026-07-04 (capital reallocated to Proteus) and the research program — `/midas-scan`, `/midas-ghost`, the convergence A/B and its 20-week death clock — was CLOSED PERMANENTLY 2026-09-01 by operator-signed directive D3. `/midas` survives only as an inert wind-down row (sleeve flat, `wind_down_complete: 2026-07-06`); it has nothing left to do and its condition can no longer fire.
