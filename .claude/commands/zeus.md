...
- Weekend dispatches: only `/midas-scan` (heavy universe scan, research-only), `/nemesis` (spinoff pipeline, gated live sleeve), and `/proteus` (discretionary god — daily cadence includes weekends as research days; markets closed means no orders) run. No `/trinity`, `/delphi`, `/achilles`, or `/midas` on weekends.
