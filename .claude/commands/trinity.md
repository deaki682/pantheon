...
1. **Load sleeves.** Read `cache/oracle_sleeve.json`, `cache/delphi_sleeve.json`, `cache/achilles_sleeve.json`. Construct sleeve objects via each class's `.load()`.

2. **Fetch live quotes.** Collect every symbol held across all three sleeves. Fetch current prices via Robinhood `get_equity_quotes`. Also fetch SPY for Delphi overlay reference.
...
- If no positions are open across all three gods, still run — the dashboard shows cash balances and equity history.
